...
**Schema Compliance**: Each new item MUST match the `outline.yaml` items schema from `/research` Step 4b — `name` (required), `description` (required, one-line context), and optional `category`. Append under the existing `items:` list; do not touch `topic`, `topic_slug`, or `execution`.
