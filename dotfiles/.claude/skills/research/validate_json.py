...
# Known aliases for the standard categories, kept only for backward compatibility
# (e.g. legacy JSON that nested under `technical_characteristics`). The authoritative
# category keys are derived dynamically from fields.yaml — see build_nested_keys().
...
def build_nested_keys(category_names):
    """Category-level JSON keys to recurse into: every category declared in fields.yaml,
    plus the known aliases of any standard category. This makes validation work for
    arbitrary domain-specific categories, not just the eight hardcoded ones."""
    nested_keys = set(category_names)
    for cat in category_names:
        nested_keys.update(BUILTIN_CATEGORY_ALIASES.get(cat, []))
    return nested_keys
...
def load_fields_yaml(fields_path):
    ...
    category_names = [category["category"] for category in data.get("field_categories", [])]
    return all_fields, required_fields, field_categories, category_names


def extract_json_fields(data, nested_keys):
    fields = set()
    stack = [(data, True)]
    while stack:
...
def validate_json(json_path, all_fields, required_fields, field_categories, nested_keys):
...
def main():
    ...
    all_fields, required_fields, field_categories, category_names = load_fields_yaml(fields_path)
    nested_keys = build_nested_keys(category_names)
    ...
    for json_path in json_files:
        if not json_path.exists():
            ...
            continue
        result = validate_json(json_path, all_fields, required_fields, field_categories, nested_keys)
